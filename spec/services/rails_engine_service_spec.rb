...
RSpec.describe RailsEngineService do
  describe "rails engine endpoint" do
    ...
    it "gets Merchant data from endpoint" do
      ...
      single_json = parsed_json[:data][0]

      expect(parsed_json).to be_a Hash
      expect(parsed_json).to have_key :data
      expect(single_json).to have_key :id
      expect(single_json).to have_key :type
      expect(single_json).to have_key :attributes
      expect(single_json[:attributes]).to have_key :name
    end
  end
end
